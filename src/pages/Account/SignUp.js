...
const SignUp = (props) => {
  ...
  const renderErrors =
    signup_error &&
    signup_error.errors.map((err) => (
      <p className="my-1 text-red-600 text-xs font-serif text-center bg-gray-100 p-1">
        {err.msg}!!
      </p>
    ));
  ...
  return (
    <div className="signup_form rounded-md w-96 min-w-min max-w-screen-md h-3/4 border-b-2 border-purple-700 shadow-2xl bg-gray-50 backdrop-filter backdrop-blur-lg bg-opacity-20">
      ...
      <p className=" text-red-400 ml-4 font-mono tracking-wide">
      ...
      <div>
        <form className="flex flex-col mx-4" onSubmit={handleSubmit}>
          ...
          <button
            disabled={!canSignUp}
            className={classNames(canSignUp ? 'bg-purple-500 text-gray-100 hover:bg-purple-700 hover:shadow-md': 'bg-purple-300 text-gray-400','p-2 text-xl border border-purple-100 rounded-md w-11/12 self-center mt-4 flex justify-center items-center')}
          >
            <span>Register</span>{" "}
            {signup_status === "loading" && <InlineLoader customStyle="ml-8" />}
